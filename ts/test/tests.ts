...
module tests {
    export class Composer {
        static compose(test: tsUnit.Test) {
            ...
            test.addTestClass(new YUIGlobalTests(), 'YUIGlobalTests');

        }
    }

    class YUIGlobalTests {
        testYUIaddMethod(context: tsUnit.TestContext) {
            var fakeYUIObject = new tsUnit.Fake<YUI>(new MyYUI());
            var target: YUI = fakeYUIObject.create();
            var fakeAppObject = new tsUnit.Fake<App>(new MyApp());
            var engine = Y.Handlebars;
            var app = fakeAppObject;
            var myApp;
            // ----------- MyYUI Application --------------
            myApp = Y.Base.create('myApp', Y.App, [], app.getClassDef(), app.getAttrsDef());
            
            var result = target.add('yui-module-name', function(Y) { Y.namespace('NamespaceName')['ClassName'] = myApp; }, 'MODULE_VERSION', { requires: ['xyz', 'xpto'] });

            context.areIdentical('yui-module-name', result.name);
        }
    }
 
    // Fake do YUI
    class MyYUI extends YUI {
        public name = 'Real';
        run() {
            return false;
        }
